...
def run_phase7(
    ...
    try:
        ...
        # ── Ensure .gitignore covers venv and cache dirs ─────────────────────
        # LLM-generated code may omit .venv/ — staging it would bloat the commit
        gitignore_path = project_dir / ".gitignore"
        required_patterns = [".venv/", "__pycache__/", "*.pyc", "*.pyo"]
        try:
            existing = gitignore_path.read_text() if gitignore_path.exists() else ""
            missing = [p for p in required_patterns if p not in existing]
            if missing:
                with gitignore_path.open("a") as gf:
                    gf.write("\n# Generated by Agentic SDLC pipeline\n")
                    gf.writelines(f"{p}\n" for p in missing)
                L(f"  .gitignore: added {missing}")
        except Exception as e:
            L(f"  Could not update .gitignore: {e}")
    ...
    if GIT_REMOTE_URL:
        ...
        remote_url = f"{GIT_REMOTE_URL.rstrip('/')}/{safe_name}.git"
